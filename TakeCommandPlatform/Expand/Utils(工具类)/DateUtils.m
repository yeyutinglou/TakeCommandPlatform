 ...
 *  2016-07-02
 */
+ (NSString *)createAtTime:(NSString *)strTime
{
    if (strTime.length==0) {
        return nil;
    }
    // _created_at == Fri May 09 16:30:34 +0800 2014====》fmt.dateFormat = @"EEE MMM dd HH:mm:ss Z yyyy";
    // 1.获得发表分享活动的发送时间
    NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
    fmt.dateFormat = @"yyyyMMdd HH:mm";
#warning 真机调试下, 必须加上这段
    fmt.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"en_US"];
    NSDate *createdDate = [fmt dateFromString:strTime];
